...
// Test that scene-based platforms (no reuse strategy) mark themselves as
// CONSUMED
TEST_F(PlatformReuseTest, ScenePlatformNoReuse) {
    ...
    auto platform =
        std::make_unique<Platform>(*game, test_rect, BLUE, false, nullptr);
...
TEST_F(PlatformReuseTest, NoReuseStrategyBehavior) {
    ...
    auto platform = std::make_unique<Platform>(
        *game, test_rect, BLUE, false, std::make_unique<NoReuseStrategy>());
