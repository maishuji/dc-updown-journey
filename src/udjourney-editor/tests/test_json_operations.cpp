...
TEST_CASE("Platform Level JSON Export", "[json][export][platform]") {
    ...
    std::string temp_path = std::filesystem::temp_directory_path() / "test_platform_export.json";
...
TEST_CASE("Platform Level JSON Import", "[json][import][platform]") {
    ...
    SECTION("Third platform (oscillating with multiple features) imported correctly") {
        ...
        bool has_spikes = std::find(p3.features.begin(), p3.features.end(), 
                                   PlatformFeatureType::Spikes) != p3.features.end();
        bool has_checkpoint = std::find(p3.features.begin(), p3.features.end(), 
...
TEST_CASE("Platform Level JSON Round Trip", "[json][roundtrip][platform]") {
    ...
    // Export to file
    std::string temp_path = std::filesystem::temp_directory_path() / "test_roundtrip.json";
    ...
    SECTION("Platform data preserved in round trip") {
        ...
        REQUIRE(imported_platform.height_tiles == original_platform.height_tiles);
        REQUIRE(imported_platform.behavior_type == original_platform.behavior_type);
...
TEST_CASE("Invalid JSON Handling", "[json][error][platform]") {
    ...
    SECTION("Missing file") {
        ...
        REQUIRE_NOTHROW(editor.test_import_platform_level_json("non_existent_file.json"));
    }
    ...
    SECTION("Unknown behavior type") {
        ...
        if (!level.platforms.empty()) {
            ...
            REQUIRE(level.platforms[0].behavior_type == PlatformBehaviorType::Static);
        }
