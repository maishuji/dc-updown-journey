...
TEST_CASE("Tilemap JSON Export", "[json][export][tilemap]") {
    ...
    for (size_t row = 0; row < level.row_cnt; ++row) {
        for (size_t col = 0; col < level.col_cnt; ++col) {
            ...
            Cell cell;
            if (row == 0 || row == level.row_cnt - 1 || col == 0 || col == level.col_cnt - 1) {
    ...
    std::string temp_path = std::filesystem::temp_directory_path() / "test_tilemap_export.json";
...
TEST_CASE("Tilemap JSON Import", "[json][import][tilemap]") {
    ...
    SECTION("Tilemap data imported correctly") {
        ...
        REQUIRE(level.tiles[12].color == GROUND_COLOR); // Ground tile in middle
        ...
        for (size_t y = 0; y < 4; ++y) {
            ...
            REQUIRE(level.tiles[y * 5 + 4].color == WALL_COLOR); // Right column
        }
    }
...
TEST_CASE("Tilemap JSON Round Trip", "[json][roundtrip][tilemap]") {
    ...
    std::vector<ImU32> pattern = {
        WALL_COLOR, WALL_COLOR, WALL_COLOR, WALL_COLOR, WALL_COLOR, WALL_COLOR,
        WALL_COLOR, AIR_COLOR, GROUND_COLOR, AIR_COLOR, AIR_COLOR, WALL_COLOR,
        WALL_COLOR, GROUND_COLOR, GROUND_COLOR, GROUND_COLOR, GROUND_COLOR, WALL_COLOR
    };
    ...
    // Export to file
    std::string temp_path = std::filesystem::temp_directory_path() / "test_tilemap_roundtrip.json";
...
TEST_CASE("Tilemap Invalid JSON Handling", "[json][error][tilemap]") {
    ...
    SECTION("Missing tilemap file") {
        ...
        REQUIRE_NOTHROW(editor.test_import_tilemap_json("non_existent_tilemap.json"));
    }
    ...
    SECTION("Invalid tile structure") {
        nlohmann::json invalid_tiles_json = {
            ...
            {"tiles", {
                ...
                {{"row", 1}, {"col", 1}, {"color", "not_a_number"}}  // Invalid color type
            }}
        };
...
TEST_CASE("Empty Level Export/Import", "[json][edge-case]") {
    ...
    // Export tilemap
    std::string tilemap_path = std::filesystem::temp_directory_path() / "empty_tilemap.json";
    editor1.test_export_tilemap_json(tilemap_path);
    
    // Export platform level
    std::string platform_path = std::filesystem::temp_directory_path() / "empty_platforms.json";
