...
struct PlatformContextPopup {
    ...
    void open(EditorPlatform* platform) {
        ...
        has_spikes =
            std::find(platform->features.begin(),
                      platform->features.end(),
                      PlatformFeatureType::Spikes) != platform->features.end();
        has_checkpoint = std::find(platform->features.begin(),
                                   platform->features.end(),
    ...
    void render() {
        ...
        if (ImGui::BeginPopupModal(
                "Platform Properties", &show, ImGuiWindowFlags_NoResize)) {
            ...
            const char* behavior_names[] = {
                "Static", "Horizontal", "Eight Turn", "Oscillating Size"};
            ...
            if (ImGui::Combo("##Behavior",
                ...
                selected_behavior =
                    static_cast<PlatformBehaviorType>(current_behavior);
            ...
            if (ImGui::SliderFloat(
                    "Width", &platform_width, 0.5f, 5.0f, "%.1f")) {
                editing_platform->width_tiles = platform_width;
            }

            if (ImGui::SliderFloat(
                    "Height", &platform_height, 0.5f, 3.0f, "%.1f")) {
            ...
            if (features_changed) {
                ...
                if (has_spikes) {
                    editing_platform->features.push_back(
    ...
    bool wants_to_delete_platform() const { return should_delete; }
