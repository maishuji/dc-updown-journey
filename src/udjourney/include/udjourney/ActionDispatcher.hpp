...
class ActionDispatcher {
 public:
    using ActionCallback = std::function<void(IGame*, const std::vector<std::string>&)>;
     ...
     */
    static void register_action(const std::string& action_name, ActionCallback callback);
 ...
 private:
     ...
     */
    static std::vector<std::string> split_action(const std::string& action_string);
};
