...
std::unordered_map<std::string, ActionDispatcher::ActionCallback> ActionDispatcher::actions_;

void ActionDispatcher::register_action(const std::string& action_name, ActionCallback callback) {
...
void ActionDispatcher::clear_actions() {
    actions_.clear();
}

std::vector<std::string> ActionDispatcher::split_action(const std::string& action_string) {
