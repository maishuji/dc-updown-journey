...
void ParticleEmitter::draw(Texture2D texture, Vector2 camera_offset) const {
    for (const auto& particle : particles_) {
        ...
        if (has_texture) {
            ...
            DrawTexturePro(texture, source, dest, origin, particle.rotation,
                           color);
        } else {
...
void ParticleEmitter::spawn_particle_() {
    ...
    particle.velocity.x = random_float(preset_.velocity_min.x, preset_.velocity_max.x);
    particle.velocity.y = random_float(preset_.velocity_min.y, preset_.velocity_max.y);
    
    // Acceleration
    particle.acceleration = preset_.acceleration;
    
    // Lifetime with variance
    float variance = random_float(-preset_.lifetime_variance, preset_.lifetime_variance);
