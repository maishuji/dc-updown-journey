...
Color parse_color(const json& j, const Color& default_color) {
    ...
    return Color{static_cast<unsigned char>(j[0].get<int>()),
                 static_cast<unsigned char>(j[1].get<int>()),
                 static_cast<unsigned char>(j[2].get<int>()),
                 j.size() > 3 ? static_cast<unsigned char>(j[3].get<int>())
...
Vector2 parse_vector2(const json& j, const Vector2& default_vec) {
    ...
    return Vector2{j.value("x", default_vec.x), j.value("y", default_vec.y)};
}
}  // namespace
...
bool ParticlePresetLoader::load_from_file(const std::string& filepath) {
    ...
    if (!udj::core::filesystem::file_exists(full_path)) {
        std::cerr << "Particle preset file not found: " << full_path
                  << std::endl;
        return false;
    }
    ...
    try {
        ...
        for (const auto& particle_json : j["particles"]) {
            ...
            if (particle_json.contains("source_rect")) {
                ...
                preset.source_rect = Rectangle{rect.value("x", 0.0f),
                                               rect.value("y", 0.0f),
            ...
            // Particle lifecycle
            preset.particle_lifetime =
                particle_json.value("particle_lifetime", 1.0f);
            preset.lifetime_variance =
                particle_json.value("lifetime_variance", 0.2f);

            // Velocity properties
            if (particle_json.contains("velocity_min")) {
                preset.velocity_min = parse_vector2(
                    particle_json["velocity_min"], Vector2{-50.0f, -100.0f});
            }
            if (particle_json.contains("velocity_max")) {
                preset.velocity_max = parse_vector2(
                    particle_json["velocity_max"], Vector2{50.0f, -50.0f});
            }
            if (particle_json.contains("acceleration")) {
                preset.acceleration = parse_vector2(
                    particle_json["acceleration"], Vector2{0.0f, 200.0f});
            }
        ...
        std::cerr << "Error loading particle presets: " << e.what()
                  << std::endl;
        return false;
    }
}

const ParticlePreset* ParticlePresetLoader::get_preset(
